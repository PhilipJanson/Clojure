...
(defn create-tree
    ...
    (if (= (count Q) 1) (first Q)
        (let [a (first Q) b (second Q) c (drop 2 Q)]
            (create-tree (insert-into-queue (make-branch a b) c))
        )
    )
...
(defn huffman-tree
    ...
    (create-tree (sort-by :frequency (map #(make-leaf % ((frequencies S) %)) (keys (frequencies S)))))
...
(defn map-huffman
    ...
    (if (isleaf? T)
        [[(T :value) seq]]
        (let [l (T :left) r (T :right)]
            (concat
                (map-huffman l (concat seq '(0)))
                (map-huffman r (concat seq '(1)))
            )
        )
    )
)
...
(defn huffman-encode
    ...
    (let [tree (huffman-tree S) huffman-map (huffman-codes tree) length (count S) bits (mapcat huffman-map S)]
